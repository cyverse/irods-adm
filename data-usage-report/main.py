...
from sqlalchemy import create_engine, text, MetaData, Table, Column, BigInteger, String, Index
...
def find_recent_json_report(max_age_days=2):
    ...
    json_files.sort(key=lambda x: extract_date(x), reverse=True)
    ...
    if age < timedelta(days=max_age_days):
        # print(age)
        print(f"Found recent report: {most_recent} (created {age.total_seconds() / 3600:.1f} hours ago)")
        return most_recent
...
def main():
    ...
    print(f"DATA ACCESS REPORT - GENERATED ON {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    parser.add_argument(
        "--ldap-server", default="ldap://ldap.iplantcollaborative.org", 
        help="LDAP server URL (default: ldap://ldap.iplantcollaborative.org)"
    )
    parser.add_argument(
        "--ldap-base", default="dc=iplantcollaborative,dc=org",
    ...
    parser.add_argument(
        "--max-report-age", type=int, default=2,
        help="Maximum age (in days) of a JSON report to reuse (default: 2)"
    )
    parser.add_argument(
        "--irods-upload-location", default="/iplant/home/shared/CyVerse_DSStats/data-products/project-data-usage"
    )
    parser.add_argument(
    ...
    try:
        ...
        with redirect_stdout(program_output):
            ...
            if args.html:
                ...
                html_output = format_html_report(report_df, args.ldap_server, args.ldap_base)
                ...
                with open(html_filename, "w") as f:
                    f.write(html_output)
                print("Report generated successfully. You can view the report in your web browser.")

                # Save it to the iRODS upload location
                if args.irods_upload_location:
                    print(f"Saving the report to iRODS upload location: {args.irods_upload_location}")
                    with iRODSSession(irods_env_file=_IRODS_ENV_FILE) as session:
                        # Create the collection if it doesn't exist
                        session.collections.create(args.irods_upload_location)
                        # Upload the file
                        session.data_objects.put(html_filename, args.irods_upload_location)
        ...
        if html_filename and args.irods_upload_location:
            ...
            print(f"To access report, please visit: https://data.cyverse.org/dav/iplant/projects/{args.irods_upload_location.replace('/iplant/home/shared', '')}/{os.path.basename(html_filename)}")
...
def create_connection_string(host: Optional[str], port: Optional[str], 
                           user: Optional[str], dbname: str) -> str:
...
def gen_report(engine, zone: str, root_resources: List[str]) -> pd.DataFrame:
    ...
    with engine.begin() as conn:  # Use transaction
        ...
        proj_data = create_proj_data_table(conn, metadata, store_resc)
        ...
        create_pub_proj_data_table(conn, metadata, proj_data, pub_obj)
...
def create_proj_data_table(conn, metadata, store_resc):
    ...
    # Create indices
    Index('proj_data_coll_data_idx', proj_data.c.coll_id, proj_data.c.data_id).create(conn)
...
def create_pub_proj_data_table(conn, metadata, proj_data, pub_obj):
    """Create temporary table for public project data using SQLAlchemy."""
    # Define the temporary table
    pub_proj_data = Table(
        'pub_proj_data', metadata,
        Column('proj', String),
        Column('data_id', BigInteger),
        Column('data_size', BigInteger),
        prefixes=['TEMPORARY']
...
def get_ldap_user_info(username: str, ldap_server: str, ldap_base: str) -> Optional[Dict]:
    ...
    try:
        ...
        result = ldap_conn.search_s(ldap_base, ldap.SCOPE_SUBTREE, search_filter, attributes)
        
        # If user found, extract attributes
        if result and len(result) > 0:
            _, attrs = result[0]
            return {
                'fullname': attrs.get('cn', [b''])[0].decode('utf-8') if attrs.get('cn') else '',
                'email': attrs.get('mail', [b''])[0].decode('utf-8') if attrs.get('mail') else '',
                'title': attrs.get('title', [b''])[0].decode('utf-8') if attrs.get('title') else '',
                'department': attrs.get('departmentNumber', [b''])[0].decode('utf-8') if attrs.get('departmentNumber') else '',
                'username': attrs.get('uid', [b''])[0].decode('utf-8') if attrs.get('uid') else '',
                'organization': attrs.get('o', [b''])[0].decode('utf-8') if attrs.get('o') else ''
            }
        ...
        for col in df.columns:
            if col == "Owner" and row[col]:
                ...
                for i, info in enumerate(owner_infos):
                    ...
                    if info['email']:
                        html += f"""<tr><td class="label">Email:</td><td><a href="mailto:{info['email']}">{info['email']}</a></td></tr>"""
                    ...
                    if info['username'] == info['fullname']:  # This is our check for missing LDAP data
