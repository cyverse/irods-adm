#!/bin/bash
#
# Usage: data-store-check.sh [HOST [PORT [CUTOFF_TIME]]]
#
...
if [ "$#" -ge 3 ]
then
  readonly CUTOFF_TIME="$3"
else
  readonly CUTOFF_TIME=$(date -I --date=today)
fi

readonly CutoffTS=$(date --date="$CUTOFF_TIME" '+0%s')

psql --host "$HOST" --port "$PORT" ICAT icat_reader <<EOF
\timing on
